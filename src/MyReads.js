    ...
    <div className="book">
      <div className="book-top">
        <div
          ...
          style={{ width: 128, height: 188, backgroundImage: `url(${imgURL})` }}
        />
      ...
      <ol className="books-grid">
        {books.map(book => (
          <li key={book.id}>
            <BooksRow
              book={book}
              change={event => props.update(book, event.target.value)}
            />
          </li>
        ))}
      </ol>
...
class MyReads extends Component {
  ...
  render() {
    let [currentlyReading, wantToRead, read] = this.filterBooksByShelf(
      this.state.allBooks
    );
    return (
      <div>
